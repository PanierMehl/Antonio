...
import pytz as pytz
from nextcord import SlashOption, Embed, Member, Interaction, Locale
...
########################################################################################################################################
    ...
    #Member-Info_Command (V4.1)
    @nc.slash_command(name="memberinfo", description="Shows all about a user", name_localizations={Locale.de: "mitgliederinformation", Locale.en_US: "memberinfo"},
    ...
    async def memberinfo(self, inter: Interaction,
                       ...
                       show_message: bool = SlashOption(name="message_visibility", description="Would you like to make the message only visible to you?",
                                                        required=False, name_localizations={Locale.en_US: "message_visibility", Locale.de: "nachrichtensichtbarkeit"},
                                                        description_localizations={Locale.en_US: "Would you like to make the message only visible to you?", Locale.de: "Möchtest du, dass die Nachricht nur für dich sichtbar ist?"})):
        
        member = _member or inter.user

        de = pytz.timezone('Europe/Berlin')
        
        memberinfo = Embed(title=self.trans["commands"]["memberinfo"]["memberinfo"]["title"][inter.locale].format(emoji_a=config.e_information, emoji_b=config.dc_members, member=member.display_name),
        ...
        if inter.guild.get_member(member.id) != None: 
            ...
            memberinfo.add_field(name=self.trans["commands"]["memberinfo"]["memberinfo"]["field_a"]["name"][inter.locale], 
            ...
            memberinfo.add_field(name=self.trans["commands"]["memberinfo"]["memberinfo"]["field_b"]["name"][inter.locale],
            ...
            await inter.edit_original_message(embed=memberinfo, view=view_r)
            ...
            memberinfo.add_field(name=self.trans["commands"]["memberinfo"]["memberinfo"]["field_c"]["name"][inter.locale], 
            ...
            await inter.edit_original_message(embed=memberinfo)   
